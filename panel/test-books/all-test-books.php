<?php if (isset($_GET['delete'])) {
    $post_id = $_GET['delete'];
    base::RunQuery("UPDATE `post` SET `post_type` = 'test-book-deleted' WHERE `post_id` = " . $post_id);
}
$specific_id = $_GET['id'];
?>
...
<div class="card">
    <h5 class="card-header">همه تست بوک ها</h5>
    ...
    $(window).ready(function () {
        ...
        if (dt_user_table.length) {
            var dt_user = dt_user_table.DataTable({
                ...
                select: true,
                ajax: {
                    url: 'API/v1/GetTestBooks.php?id=<?php echo $specific_id; ?>'
                },
                columns: [
                    {
                        data: 'title'
