<?php
$error = false;
if (isset($_GET['test-book']) && $_GET['test-book'] > 0) {
    ...
    $test_book_answers = $parent->get_meta('test_book_answers');
    if ($negative_score === false)
        $negative_score = 'ندارد';
    else
        $negative_score = 'دارد';
    $lessons = json_decode($parent->get_meta('lessons'), true);
    $test_book_predictions = $parent->get_meta('test_book_prediction');
    $test_book_prediction = 'نامشخص';
    if (!empty($test_book_predictions)) {
        $test_book_predictions = json_decode($test_book_predictions, true);
        $matched = false;
        $lowestMin = PHP_INT_MAX;
        $highestRankForMin = null;
        $score = round($score);
        foreach ($test_book_predictions as $score_range => $rank_range) {
            ...
            if ($score >= $min && $score <= $max) {
                $test_book_prediction = $rank_range;
                $matched = true;
                break;
            }
        }
        if (!$matched && $score < $lowestMin) {
            $test_book_prediction = $highestRankForMin . '+';
        }
    }
    ...
    $rank_q = "WITH author_scores AS (SELECT author, MAX(post_excerpt*1) AS top_score FROM post
        WHERE `post_type`= 'test_book_result'
    ...
    <div class="col-12 md-4">
        <div class="card-body card">
            <div class="row justify-content-between g-3">
                ...
                <div class="col-6 col-md-2">
                    <div class="card p-2">
                        <h6>تخمین رتبه: <b><?php echo $test_book_prediction; ?></b></h6>
                    </div>
                </div>
                <div class="col-6 col-md-2">
                    <div class="card p-2">
                ...
                <?php if ($now > $term_end): ?>
                    <div class="col-12">
                        <a href="<?php echo $test_book_answers; ?>" class="btn btn-primary" download>پاسخنامه تشریحی</a>
                    </div>
                <?php endif; ?>
            </div>
        </div>
                            ...
                            <div class="row mb-4">
                                <div class="col-4">
                                    <div class="card p-2 p-md-3">
                                        <h5>درصد : <b><?php echo $raw; ?></b></h5>
                                    </div>
                                </div>
                                <div class="col-4">
                                    <div class="card p-2 p-md-3">
                                        <h5>تراز : <b><?php echo $score; ?></b></h5>
                                    </div>
                                </div>
                                <div class="col-4">
                                        ...
                                        <div class="accordion-collapse collapse <?php if ($j == 1)
                                            ...
                                            <div class="accordion-body d-flex align-items-center flex-wrap">
                                                ...
                                                <?php if (!empty($answer_video)) { ?>
                                                    <a class="btn btn-primary btn-sm p-2  d-flex align-items-center"  href="javascript:void(0);" data-bs-toggle="modal"
