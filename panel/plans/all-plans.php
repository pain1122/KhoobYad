...
if (isset($_GET['delete'])) {
    ...
    if ($plan_id > 0) {
        $delete_q = "UPDATE `post` SET `post_status` = 'deactive' WHERE `post_id` = $plan_id AND `post_type` = 'daily-plan'";
    ...
    $(window).ready(function() {
        ...
        if (dt_user_table.length) {
            var dt_user = dt_user_table.DataTable({
                ...
                select: true,
                ajax: {
                    url: 'API/v1/GetStudyPlans.php?id=<?php echo $uid; ?>'
                },
                columns: [
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            var $name = full['name'],
                                $id = full['user_id'];
                            var $row_output =
                                '<a href="index.php?page=plans/my-plans.php&uid=' + $id + '">' +
                    ...
                    },
                    {
                        ...
                        orderable: false,
                        render: function(data, type, full, meta) {
                            var $adviser = full['adviser'];
                            return "<span class='text-truncate d-flex align-items-center'>" + $adviser + '</span>';
                        }
                    },
                    {
                        // op
