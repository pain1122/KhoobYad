...
if (isset($_POST['submit']) || isset($_POST['draft'])) {
    ...
    $obj->set_taxonomy('class_book_tag', $post_tags);
    $obj->set_taxonomy('class_book_category', $categories);
    $obj->set_post_type('class-book');
    ...
    $selected_users = [];
    $selected_users = base::FetchArray("SELECT `author` FROM `post` 
    INNER JOIN `items_order` ON `items_order`.`order_id` = `post`.`post_id` 
    WHERE `items_order`.`item_id` = $post_id 
    ...
    if (!isset($_GET['id'])) {
        $url = site_url . "panel/index.php?page=product/add-class-book.php&id=" . $post_id;
        base::redirect($url);
    }
}
$selected_users = [];
if (isset($_GET['id'])) {
    ...
    if ($role == 'admin' || $role == 'school') {
        $selected_users_array = base::FetchArray("SELECT `author`,`post_id` FROM `post` 
        INNER JOIN `items_order` ON `items_order`.`order_id` = `post`.`post_id` 
        WHERE `items_order`.`item_id` = $post_id AND `post_status` = 'completed' 
        GROUP BY `author`;");
...
$all_cats = tag::get_taxonomies(['type' => 'class_book_category']);
$all_tags = tag::get_taxonomies(['type' => 'class_book_tag']);
$students = base::FetchArray("SELECT `users`.`user_id` FROM `users` 
INNER JOIN `user_meta` ON `users`.`user_id` = `user_meta`.`user_id` 
WHERE `key` = 'role' AND `value` = 'student'");
$teachers = base::FetchArray("SELECT `users`.`user_id`,`nicename` FROM `users` INNER JOIN `user_meta` ON `users`.`user_id` = `user_meta`.`user_id` WHERE `key` = 'role' AND `value` IN ('teacher','adviser','support')");
        ...
        <div class="accordion" id="collapsibleSection">
            <div class="card">
                <div class="card-body">
                    ...
                    <?php if (is_countable($teachers) && count($teachers)) : ?>
                        ...
                        <select name="teacher" class="select2">
                            ...
                            <?php foreach ($teachers as $teacher) : ?>
                                <option value='<?php echo $teacher['user_id']; ?>' <?php if ($teacher['user_id'] == $master) echo 'selected'; ?>><?php echo $teacher['nicename']; ?></option>;
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
                    ...
                    <label class="form-label">لینک آزمون</label>
                    <input type="text" name="exam" value="<?php echo $exam; ?>" class="form-control" placeholder="لینک آزمون خود را وارد کنید">
                    <label class="form-label">لینک تست بوک</label>
                    <input type="text" name="testbook" value="<?php echo $testbook; ?>" class="form-control" placeholder="لینک تست بوک خود را وارد کنید">
