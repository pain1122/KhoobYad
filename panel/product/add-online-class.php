...
if (isset($_POST['submit']) || isset($_POST['draft'])) {
    ...
    $obj->set_post_type('online-class');
    $post_id = $obj->get_id();
    $post_status = 'publish';
    ...
    $obj->set_taxonomy('online_class_tag', $post_tags);
    $obj->set_taxonomy('online_class_category', $categories);
    ...
    if (!isset($_GET['id'])) {
        $url = site_url . "panel/index.php?page=product/add-online-class.php&id=" . $post_id;
        base::redirect($url);
    }
}
$selected_users = [];
...
$all_cats = tag::get_taxonomies(['type' => 'online_class_category']);
$all_tags = tag::get_taxonomies(['type' => 'online_class_tag']);
                    ...
                    <?php if (is_countable($teachers) && count($teachers)) : ?>
                        ...
                        <select name="teacher" class="select2">
                            ...
                            <?php foreach ($teachers as $teacher) : ?>
                                <option value='<?php echo $teacher['user_id']; ?>' <?php if ($teacher['user_id'] == $master) echo 'selected'; ?>><?php echo $teacher['display_name']; ?></option>;
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
            ...
            ?>
                    <div class="card-body">
                        <div class="d-flex flex-wrap align-items-center">
                            ...
                            <input type="text" name="session[<?php echo $sessions_count; ?>][date]" value="<?php echo $date; ?>" onkeyup="replace_digits(this);" class="form-control mb-3" placeholder="تاریخ جلسه خود را وارد کنید">
                            <hr>
...
<div class="offcanvas offcanvas-start" tabindex="-1" id="offcanvasAddTaxonomy" aria-labelledby="offcanvasAddTaxonomyLabel">
    ...
    <div class="offcanvas-body mx-0 flex-grow-0">
        <form class="add-new-user pt-0" id="addNewTaxonomy" data-taxonomy='online_class_category' onsubmit="return false">
            <div class="mb-3">
            ...
            <div class="mb-3">
                <label class="form-label">نامک</label>
                <input type="text" onkeyup="validate_name(this,'online_class_category')" onchange="replace_digits(this);validate_name(this,'online_class_category')" id="type" class="form-control text-start" placeholder="نامک" name="slug" required>
            </div>
            <div class="mb-3" id="category-parent">
                <label class="form-label">دسته مادر</label>
                <input name="online_class_category_parent" class="form-control parent_selection" placeholder="انتخاب دسته">
            </div>
...
<script>
    ...
    const multiple_select2 = $('.multiple.select2');
    if (multiple_select2.length) {
        multiple_select2.each(function() {
            var $this = $(this);
            $this.wrap('<div class="position-relative"></div>').select2({
                placeholder: 'انتخاب',
                multiple: true,
    ...
    $('#add-tag').click(function() {
        $('#addNewTaxonomy').attr('data-taxonomy', 'online_class_tag');
        $('#offcanvasAddTaxonomy #offcanvasAddTaxonomyLabel span').text('برچسب');
        $('#offcanvasAddTaxonomy #type').attr("onkeyup", "validate_name(this,'online_class_tag')");
        $('#offcanvasAddTaxonomy #type').attr("onchange", "validate_name(this,'online_class_tag')");
        $('#offcanvasAddTaxonomy #category-parent').hide();
    });
    $('#add-category').click(function() {
        $('#addNewTaxonomy').attr('data-taxonomy', 'online_class_category');
        $('#offcanvasAddTaxonomy #offcanvasAddTaxonomyLabel span').text('دسته بندی');
        $('#offcanvasAddTaxonomy #type').attr("onkeyup", "validate_name(this,'online_class_category')");
        $('#offcanvasAddTaxonomy #type').attr("onchange", "validate_name(this,'online_class_category')");
        ...
        getJSON('API/v1/InsertTaxonomy.php?name=' + name + '&slug=' + slug + '&parent=' + parent + '&type=' + type, function(err, data) {
            if (data != null && data.length > 0) {
                ...
                if (data == 'exsists') {
                    ...
                    if (data[0]['type'] == 'online_class_category') {
                        cats.push({
                            "name": name,
                            "value": id
                        });
