...
if (isset($_POST['submit']) || isset($_POST['draft'])) {
    ...
    $obj->set_taxonomy('off_boxes_tag',$post_tags);
    $obj->set_taxonomy('off_boxes_category',$categories);
    ...
    INNER JOIN `items_order` ON `items_order`.`order_id` = `post`.`post_id`
     WHERE `items_order`.`item_id` = $post_id AND `post_status` = 'completed' GROUP BY `author`;");
    ...
    if (!isset($_GET['id'])) {
        $url = site_url . "panel/index.php?page=product/add-off-boxes.php&id=" . $post_id;
        base::redirect($url);
    }
}
$selected_users = [];
if (isset($_GET['id'])) {
    ...
    if ($role == 'admin' || $role == 'school') {
        $selected_users_array = base::FetchArray("SELECT `author` FROM `post` 
        INNER JOIN `items_order` ON `items_order`.`order_id` = `post`.`post_id` 
        WHERE `items_order`.`item_id` = $post_id 
        AND `post_status` = 'completed' GROUP BY `author`;");
        foreach ($selected_users_array as $user) {
            $selected_users[] = $user['author'];
        }
    }
}
$all_cats = tag::get_taxonomies(['type' => 'off_boxes_category']);
$all_tags = tag::get_taxonomies(['type' => 'off_boxes_tag']);
            ...
            ?>
                    <div class="card-body">
                        <div class="d-flex flex-wrap align-items-center">
                            ...
                            <input type="text" name="session[<?php echo $sessions_count; ?>][price]" value="<?php echo $title; ?>" onkeyup="replace_digits(this);" class="form-control mb-3" placeholder="قیمت جلسه خود را وارد کنید">
