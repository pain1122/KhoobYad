    ...
    if ($role == 'student') {
      ...
      if ((is_countable($user_classes) && preg_match('/"' . preg_quote($post_id, '/') . '"/i', json_encode($user_classes))) || ($user_order_items && array_search($post_id, $user_order_items)) || $price === 0  || ($subscription == 'exam' || $subscription == 'all')) {
        if (in_array($post_id, $exams)) {
          $button = '<a class="btn btn-sm btn-primary" href="?page=exams/rankings.php&exam=' . $post_id . '">نمایش کارنامه</a>';
        } else if ($time < $term_start || $time >= $term_end || $status == 'finished') {
          $button = "<span class='btn btn-sm btn-secondary'>غیر فعال</span>";
        } else {
          $button = "<a href='?page=exams/view.php&exam=$post_id' class='btn btn-sm btn-primary'>شرکت در آزمون</a>";
        }
      } else {
      ...
      <div class="dropdown-menu">
        <a class="dropdown-item" href="?page=exams/view.php&exam=' . $post_id . '" target="_blank"><i class="fa-regular fa-eye me-1"></i> نمایش</a>
        <button type="button" value="https://my10020.ir/panel/index.php?page=exams/all-exams.php&id=' . $post_id . '" class="dropdown-item" onclick="copyToClipBoard(this)" ><i class="fa-regular fa-share me-1"></i> اشتراک گذاری</button>
        <a class="dropdown-item" href="?page=exams/rankings.php&exam=' . $post_id . '" target="_blank"><i class="fa-solid fa-ranking-star"></i> نمایش رتبه بندی</a>
        <a class="dropdown-item" href="index.php?page=exams/add-exam.php&id=' . $post_id . '" target="_blank"><i class="bx bx-edit-alt me-1"></i> ویرایش</a>
        <a class="dropdown-item" href="index.php?page=exams/all-exams.php&delete=' . $post_id . '"><i class="bx bx-trash me-1"></i> حذف</a>
      </div>
    </div>';
    }
