...
if (isset($_POST['submit']) || isset($_POST['draft'])) {
    ...
    $exam_scale = $_POST['exam-scale'];
    $exam_rank = $_POST['exam-rank'];
    ...
    if (is_countable($exam_rank) && count($exam_rank) > 0){
        for ($i = 0; $i < count($exam_rank); $i++) {
            $exam_prediction[$exam_scale[$i]] = $exam_rank[$i];
        }
        $exam_prediction = json_encode($exam_prediction, JSON_UNESCAPED_UNICODE);
    }else{
        $exam_prediction = '';
    }
    $metas = [
    ...
    if (!isset($_GET['id'])) {
        $url = site_url . "panel/index.php?page=exams/add-exam.php&id=" . $post_id;
        base::redirect($url);
    }
}
if (isset($_GET['id'])) {
    ...
    $exam_questions = $obj->get_meta('exam_questions');
    $exam_answers = $obj->get_meta('exam_answers');
    $exam_prediction = json_decode($obj->get_meta('exam_prediction'), true);
    ...
    <div class="col-12 col-lg-7">
        <div class="card mb-4">
            <div class="card-body">
                ...
                <div class="card-footer row" id="exam-scales">
                    <?php foreach ($exam_prediction as $scale => $rank) { ?>
                        <div class="col-12 col-md-5 mb-3">
                            <label class="form-label">بازه تراز</label>
                            <input type="text" name="exam-scale[]" value="<?php echo $scale; ?>" placeholder="مثلا 0-1000"
                                onkeyup="replace_digits(this);" class="form-control" required>
                        </div>
                        <div class="col-12 col-md-5 mb-3">
                            <label class="form-label">بازه رتبه</label>
                            <input type="text" name="exam-rank[]" value="<?php echo $rank; ?>" placeholder="مثلا 0-1000"
                                onkeyup="replace_digits(this);" class="form-control" required>
                        </div>
    ...
    <div class="col-12 col-lg-5">
        <div class="card mb-4">
            ...
            <div class="card-body d-flex justify-content-around p-3 pt-0">
                ...
                <button type="submit" name="draft" class="btn btn-primary p-2"><i class="fa-regular fa-square-pen"></i>
                    پیش نویس</button>
                <?php if (!empty($guid)): ?>
                    <a href="/panel/index.php?page=exams/view.php&exam=<?php echo $post_id; ?>" class="btn btn-info p-2"><i
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <div class="form-check mb-2">
                    <label class="form-check-label"> آزمون نمره منفی دارد ؟</label><input class="form-check-input"
        ...
        <div class="card mb-4">
            <h5 class="card-header d-flex align-items-center">انتخاب فایل سوال های آزمون<a
                    ...
                    href='filemanager/dialog.php?type=1s&field_id=exam_questions' data-fancybox data-type='iframe'
                    data-preload='false'>انتخاب فایل</a></h5>
            <div class="card-body">
                <input type='text' name='exam_questions' class='form-control mb-3' id='exam_questions'
                    value="<?php echo $exam_questions; ?>">
            </div>
        </div>
        <div class="card mb-4">
            <h5 class="card-header d-flex align-items-center">انتخاب فایل جواب های آزمون<a
                    class='btn btn-sm btn-success mr-auto mb-2'
                    href='filemanager/dialog.php?type=1s&field_id=exam_answers' data-fancybox data-type='iframe'
                    data-preload='false'>انتخاب فایل</a></h5>
            <div class="card-body">
                <input type='text' name='exam_answers' class='form-control mb-3' id='exam_answers'
                    value="<?php echo $exam_answers; ?>">
            </div>
        </div>
    </div>
</form>
...
<script>
    ...
    $("#add-overall-scale").click(function () {
        $(this).before(`<div class="col-12 col-md-5 mb-3">
                        <label class="form-label">بازه تراز</label>
                        <input type="text" name="exam-scale[]" placeholder="مثلا 0-1000" onkeyup="replace_digits(this);" class="form-control" required>
                    </div>
                    <div class="col-12 col-md-5 mb-3">
                        <label class="form-label">بازه رتبه</label>
                        <input type="text" name="exam-rank[]" placeholder="مثلا 0-1000" onkeyup="replace_digits(this);" class="form-control" required>
                    </div>
                    <div class="col-12 col-md-2 mb-3"><button type="button" class="btn btn-sm btn-danger mr-3" onclick="$(this).parent().prev().remove();$(this).parent().prev().remove();$(this).parent().remove();">حذف</button></div>`)
    });
