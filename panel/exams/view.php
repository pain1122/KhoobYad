...
if (isset($_GET['exam']) && $uid > 0) {
    ...
    elseif (in_array($_GET['exam'], $exams)) {
        ...
        window.location.replace('" . site_url . "panel/index.php?page=exams/rankings.php&exam=" . $_GET['exam'] . "');
        </script>";
    }
...
if (isset($_POST['submit'])) {
    ...
    $record->set_post_type('exam_result');
    ...
    $user->insert_user_meta(['exams' => json_encode($exams, JSON_PRETTY_PRINT || JSON_UNESCAPED_UNICODE)]);
    base::redirect(site_url . "panel/index.php?page=exams/result.php&exam=$record_id");
}
?>
<script>
...
<div class="row g-3">
            ...
            <div class="row g-3">
                ...
                <div class="mt-4 col-12 col-lg-2">
                    <button type="submit" onclick="$('#form-submit').trigger('click');" name="submit"
                        class="btn w-100 btn-primary p-2"><i class="fa-regular fa-pen-to-square"></i> ثبت جواب ها</button>
                </div>
            </div>
        </div>
    </div>
    <?php if ($error == false || $role != 'student'): ?>
        <div class="col-12 col-lg-7">
            <div class="_df_book" source="<?php echo $exam_questions; ?>"></div>
        </div>
        <div class="col-12 col-lg-5" id="answers-sheet">
            <form action="" method="post" class="card-body card p-2 p-md-4 pb-5" id="exam-form"
